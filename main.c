...
//* First we check if player_no has a valid move it can make, i.e. it has a penguin on the GS.map that can move. We use movement_possibility_check function for this:
//
//     if (movement_possibility_check(rows, columns, GS.map[rows][columns], GS.players[], player_no) == 1)
//     {
...
//
//         while (map[selected_row][selected_column].penguin_owner != player_no || floe_availability_check(rows, columns, GS.map[rows][columns], selected_row, selected_column) == 0)
//         {
...
//
//             while (destination_validity_check(rows, columns, GS.map[rows][columns], selected_row, selected_column, destination_row, destination_column) == 0)
//             {
...
//             }
//
//             // So now that the destination is a valid choice we move the penguin i.e. update the GS.map and the scoreboard to reflect the move. We use move_penguin function for this.
//
//             move_penguin(rows, columns, GS.map[rows][columns], selected_row, selected_column, destination_row, destination_column, player GS.players[], player_no);
//
