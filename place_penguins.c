...
int place_penguins(GameState GS)
{
    ...
    for (current_penguin = 1; current_penguin <= total_penguins; current_penguin++)
    {
       ...
       //For Interactive Mode   get_placement_coordinates(GS, &placement_coordinates, current_penguin); // Get placement coordinates from the player.
       /*For Auto Mode*/  placement_score_check(GS,&placement_coordinates);
